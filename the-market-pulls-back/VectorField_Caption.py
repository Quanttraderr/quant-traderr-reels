...
KEYWORD = "FIELD"                       # the comment-keyword CTA

CAPTION = """
Most traders ask where the market is going. A quant asks where it usually goes from here, which is the version of the question the tape can actually answer.

Three numbers place SPY on any session: 20 day momentum, 20 day realised volatility, and how far it sits below its previous high. 34 years, 8,426 states, cut into a grid. For every cell holding at least 30 sessions, the arrow is the average move over the following 20 sessions.

41 of the 43 filled cells point back toward the middle, 95% of them. Wherever the market stands, the average next month pulls it back toward ordinary.

The catch: the particles are tracers in an average field, not market paths. An average is not a rule, the spread around every arrow is far wider than the arrow itself, and some of this pull is mechanical, because a standardised number cannot run from its own mean forever.

Comment "FIELD" and I'll send the code that builds it.

Follow @quant.traderr for daily breakdowns of the math behind the markets.
"""

HASHTAGS = ("#quant #trading #finance #markets #meanreversion #volatility "
            "#quantfinance #algotrading #datascience #python")

ALT = (
    "Slide 1: Hundreds of glowing tracers streaming through a black 3D space, "
    "leaving coloured trails. The three axes are 20 day momentum, 20 day "
    "realised volatility and drawdown from the previous high for the S&P 500 "
    "ETF. Short arrows mark the average move over the next 20 sessions from "
    "each region of that space, coloured blue through red by how strong the "
    "pull is, and almost all of them point inward, so the tracers drift toward "
...
def _checks(text, tags_line, figs):
    ...
    for key in ("share_inward", "n_cells", "n_inward", "n_states", "horizon",
                "window_days", "n_years", "min_obs"):
