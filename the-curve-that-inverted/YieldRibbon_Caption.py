...
KEYWORD = "CURVE"                       # the comment-keyword CTA

CAPTION = """
Retail hears "inverted yield curve" and reaches for the sell button. The curve then stayed inverted for two years.

Twenty years of US Treasuries, 5,028 sessions, 4 quoted maturities from 13 weeks to 30 years. One strand per session, stacked by date, so the ribbon is the curve itself and the twist running through it is the inversion.

The longest unbroken inversion ran 503 sessions, Nov 2022 to Nov 2024. It went deepest in May 2023, with the 10 year yielding 1.70 points less than the 13 week. Across the whole sample the curve sat inverted on 15% of sessions.

The catch: this is not a timing signal, and this reel deliberately shows no market return next to the curve. Twenty years hold a handful of episodes, nowhere near enough to conclude what follows one.

Comment "CURVE" and I'll send the code that builds it.

Follow @quant.traderr for daily breakdowns of the math behind the markets.
"""

HASHTAGS = ("#quant #trading #finance #markets #bonds #yieldcurve "
            "#macro #quantfinance #datascience #investing")

ALT = (
    "Slide 1: A ribbon of thin coloured strands climbing through a black 3D "
    "space. Each strand is the US Treasury yield curve on one session, running "
    "from 13 week bills on the left to 30 year bonds on the right, stacked by "
    "date so height is time. Strands are coloured blue through red by the 10 "
    "year minus 13 week spread, red where the curve is inverted, and the "
    "ribbon visibly twists over during the long red stretch."
)
...
def _checks(text, tags_line, figs):
    ...
    for key in ("longest", "inv_start", "inv_end", "share_inverted", "deepest",
                "deepest_date", "n_sessions", "n_maturities", "mat_short",
                "mat_bench", "mat_long"):
