...
RUN
    python BellCurve_Static_Pipeline.py
    -> BellCurve_Static.png  (3240x5760; 10.8x19.2in at 300 DPI)
"""
import os

from BellCurve_Reel_Pipeline import (CONFIG, BASE_DIR, fetch, compute,
                                        validate, prepare_geometry,
                                        render_frame, log)

STATIC = {"W": 3240, "H": 5760, "DPI": 300, "ELEV": 16, "AZIM": -40}
...
def render_static(out):
    ...
    # One curated angle instead of wherever the sweep happens to end. The sine
    # on the elevation vanishes at the last frame, so setting the base values
    # fixes the camera exactly.
...
if __name__ == "__main__":
    render_static(os.path.join(BASE_DIR, "BellCurve_Static.png"))
